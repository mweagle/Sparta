...
type buildContext struct {
	...
	// literals or Base64 ref functions
...
// create a new gof.Parameter struct and return it...
func newStackParameter(paramType string,
	description string,
	defaultValue string,
	allowedPattern string,
	minLength int) gof.Parameter {

	return gof.Parameter{
		Type:           paramType,
...
func callServiceDecoratorHook(lambdaFunctionCode *goflambda.Function_Code,
	userdata *userdata,
	buildContext *buildContext,
	logger *zerolog.Logger) error {
	if userdata.workflowHooks == nil {
	...
	for eachIndex, eachServiceHook := range userdata.workflowHooks.ServiceDecorators {
		...
		serviceTemplate := gof.NewTemplate()
...
func callValidationHooks(validationHooks []ServiceValidationHookHandler,
	...
	if len(validationHooks) != 0 {
		jsonBytes, jsonBytesErr := template.JSON()
		if jsonBytesErr != nil {
...
func (viro *verifyIAMRolesOp) Invoke(ctx context.Context, logger *zerolog.Logger) error {
	...
	for _, eachLambdaInfo := range viro.userdata.lambdaAWSInfos {
		...
		if nil != eachLambdaInfo.RoleDefinition {
			...
			if !exists {
				...
				viro.buildContext.cfTemplate.Resources[logicalName] =
					eachLambdaInfo.RoleDefinition.toResource(eachLambdaInfo.EventSourceMappings,
						eachLambdaInfo.Options,
						logger)

				viro.buildContext.lambdaIAMRoleNameMap[logicalName] = gof.GetAtt(logicalName, "Arn")
			}
		}
		...
		for _, eachCustomResource := range eachLambdaInfo.customResources {
			if nil != eachCustomResource.roleDefinition {
				...
				if !exists {
					viro.buildContext.cfTemplate.Resources[customResourceLogicalName] =
						eachCustomResource.roleDefinition.toResource(nil,
							eachCustomResource.options,
							logger)
					viro.buildContext.lambdaIAMRoleNameMap[customResourceLogicalName] = gof.GetAtt(customResourceLogicalName, "Arn")
				}
			}
		}
	}
	...
	for _, eachRoleName := range allRoleNames {
		...
		if !exists {
			...
			viro.buildContext.lambdaIAMRoleNameMap[eachRoleName] = *resp.Role.Arn
		}
	}
...
func (cpo *createPackageOp) buildZIPArchive(sanitizedServiceName string,
	...
	if (cpo.userdata.s3SiteContext != nil &&
		cpo.userdata.s3SiteContext.s3Site != nil) || cpo.userdata.dockerFile == "" {
...
func (cto *createTemplateOp) insertTemplateParameters(ctx context.Context,
	logger *zerolog.Logger) (map[string]string, error) {

	// Code archive...
	if cto.buildContext.cfTemplate.Parameters == nil {
		cto.buildContext.cfTemplate.Parameters = make(map[string]gof.Parameter)
	}
	paramRefMap := make(map[string]string)
	...
	cto.buildContext.cfTemplate.Parameters[StackParamArtifactBucketName] = newStackParameter(
		...
		".+",
		3)
	paramRefMap[StackParamArtifactBucketName] = gof.Ref(StackParamArtifactBucketName)
	...
	if cto.userdata.dockerFile == "" {
		...
		cto.buildContext.cfTemplate.Parameters[StackParamS3CodeKeyName] = newStackParameter(
			...
			".+",
			3)
		paramRefMap[StackParamS3CodeKeyName] = gof.Ref(StackParamS3CodeKeyName)
		...
		cto.buildContext.cfTemplate.Parameters[StackParamS3CodeVersion] = newStackParameter(
			...
			".*",
			0)
		paramRefMap[StackParamS3CodeVersion] = gof.Ref(StackParamS3CodeVersion)
	} else {
	...
	if nil != codePipelineEnvironments {
		for _, eachEnvironment := range codePipelineEnvironments {
			for eachKey := range eachEnvironment {
				cto.buildContext.cfTemplate.Parameters[eachKey] = gof.Parameter{
					Type:    "String",
					Default: "",
				}
			}
	...
	// Add the output
	cto.buildContext.cfTemplate.Outputs[StackOutputBuildTime] = gof.Output{
		Description: "UTC time template was created",
		Value:       time.Now().UTC().Format(time.RFC3339),
	}
	cto.buildContext.cfTemplate.Outputs[StackOutputBuildID] = gof.Output{
		Description: "BuildID",
		Value:       cto.userdata.buildID,
	}
...
func (cto *createTemplateOp) ensureDiscoveryInfo(ctx context.Context, logger *zerolog.Logger) error {
	...
	requiredEnvVars := []string{envVarDiscoveryInformation, envVarLogLevel}
	...
	for eachResourceID, eachResourceDef := range cto.buildContext.cfTemplate.Resources {
		switch typedResource := eachResourceDef.(type) {
		case *goflambda.Function:
			if typedResource.Environment == nil {
				...
				if typedResource.Environment.Variables == nil {
					typedResource.Environment.Variables = make(map[string]string)
				}
				for _, eachKey := range requiredEnvVars {
					_, exists := typedResource.Environment.Variables[eachKey]
					if !exists {
						validateErrs = append(validateErrs,
...
func (cto *createTemplateOp) Invoke(ctx context.Context, logger *zerolog.Logger) error {
	...
	if !isDockerPackage {
		s3CodeResource.S3Bucket = paramMap[StackParamArtifactBucketName]
		s3CodeResource.S3Key = paramMap[StackParamS3CodeKeyName]
		s3CodeResource.S3ObjectVersion = paramMap[StackParamS3CodeVersion]
	} else {
		s3CodeResource.ImageUri = paramMap[StackParamCodeImageURI]
	}
	...
	// if it's defined, and then merge it with the normal output map.
	apiGatewayTemplate := gof.NewTemplate()
	...
	if nil != cto.userdata.s3SiteContext.s3Site {
		exportErr := cto.userdata.s3SiteContext.s3Site.export(cto.userdata.serviceName,
			...
			gof.Ref(StackParamArtifactBucketName),
			s3CodeResource,
			gof.Ref(StackParamS3SiteArchiveKey),
	...
	cfTemplateJSON, cfTemplateJSONErr := cto.buildContext.cfTemplate.JSON()
